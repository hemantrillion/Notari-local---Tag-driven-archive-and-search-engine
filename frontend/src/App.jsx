  ...
  return (
    <div className="split-layout">
      ...
      <div className="app-side" onClick={handleBackgroundClick}>
        ...
        />
        <div className={`sidebar-drawer ${drawerOpen ? 'open' : ''}`} onClick={(e) => e.stopPropagation()}>
...
{/* Search Box absolute centered */}
        ...
        {/* 90% Screen Height Tag Editor Modal */}
        {activeEditLinkId !== null && (
          <div 
            className="tag-editor-overlay" 
            onClick={(e) => {
              e.stopPropagation(); // Prevent bubbling up to background click
              setActiveEditLinkId(null);
            }}
          >
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '16px', overflowY: 'auto' }}>
                ...
                {/* Side-by-side URL ID and Source URL */}
                 <div style={{ display: 'flex', gap: '12px' }}>
                   <div style={{ flex: 1 }}>
                     <label className="input-label" style={{ fontWeight: 'bold', fontSize: '11px', display: 'block', marginBottom: '4px' }}>
                       URL ID (Identifier)
                     </label>
                     <input 
                       type="text" 
                       className="input-field" 
                       style={{ fontFamily: 'monospace', padding: '6px 8px', fontSize: '12px', backgroundColor: '#f1f3f4', cursor: 'not-allowed', color: '#5f6368', width: '100%', boxSizing: 'border-box' }}
                       value={editUrlId} 
                       readOnly
                     />
                   </div>
                   <div style={{ flex: 1.5 }}>
                     ...
                     <div 
                       style={{ 
                        ...
                        title={links.find(l => l.id === activeEditLinkId)?.url}
                      >
                        {links.find(l => l.id === activeEditLinkId)?.url}
                      </div>
                    </div>
                  </div>
                    ...
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                      {editTags && editTags.length > 0 ? (
                        editTags.map((t) => (
                          <span 
                            key={t.code}
                            className="tag-pill inline-tag-pill"
                            style={{
                            ...
                            onClick={() => {
                              const nextTags = editTags.filter(tag => tag.code !== t.code);
                              updateActiveTags(nextTags);
                            }}
                          >
                            {t.label}
                            <span 
                              className="tag-remove-cross" 
                              style={{
                                fontWeight: 'bold',
                                fontSize: '14px',
                                lineHeight: '1',
                  ...
                  {/* 2. Tag Label Input with Custom Suggestion Dropdown */}
                  {isCreatingTagInline ? (
                    <div className="input-group">
                      <label className="input-label" style={{ fontWeight: 'bold', fontSize: '12px', display: 'block', marginBottom: '4px' }}>
                      ...
                      <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                        <input 
                          ...
                          style={{ flex: 1, height: '32px', padding: '6px', fontSize: '12px' }}
                          value={inlineTagName}
                          ...
                          onClick={async () => {
                            ...
                            try {
                              const res = await fetch(`${API_BASE}/tags`, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ label: inlineTagName.trim() })
                              });
                              const data = await res.json();
                              if (res.ok && data.data) {
                                await fetchTags();
                                if (!editTags.some(t => t.code === data.data.code)) {
                                  updateActiveTags([...editTags, data.data]);
                                }
                    ...
                    <div className="input-group" style={{ position: 'relative' }}>
                      <label className="input-label" style={{ fontWeight: 'bold', fontSize: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                        <span>Tag Label</span>
                        <span 
                          style={{ color: '#1a73e8', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold' }}
                          onClick={() => {
                      ...
                      <input 
                        ...
                        onKeyDown={(e) => {
                          const filtered = tags.filter(t => 
                            t.label.toLowerCase().includes(tagSearchQuery.toLowerCase().trim())
                          );
                          if (filtered.length > 0) {
                            if (e.key === 'ArrowDown') {
                              e.preventDefault();
                              ...
                              if (activeSuggestionIndex >= 0 && activeSuggestionIndex < filtered.length) {
                                ...
                                if (selectedTag.code !== '0000') {
                                  if (!editTags.some(t => t.code === selectedTag.code)) {
                                    updateActiveTags([...editTags, selectedTag]);
                                  }
                                }
                                setTagSearchQuery('');
                            ...
                            if (filtered.length === 0) {
                              return (
                                <div style={{ padding: '8px 12px', fontSize: '12px', color: 'var(--text-muted)' }}>
                                  No matching tags (will create new)
                                </div>
                              );
                            }
                            return filtered.map((t, idx) => {
                              ...
                              return (
                                <div 
                                  ...
                                  onClick={() => {
                                    if (t.code !== '0000') {
                                      if (!editTags.some(tag => tag.code === t.code)) {
                                        updateActiveTags([...editTags, t]);
                                      }
                                    }
                                    setTagSearchQuery('');
                                    setShowSuggestions(false);
                ...
                {/* 3. Helper Selectors for Type & Source */}
                <div style={{ display: 'flex', gap: '12px' }}>
                  <div style={{ flex: 1 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    >
                      <option value="ytb">YouTube (ytb)</option>
                      <option value="ins">Instagram (ins)</option>
                      <option value="gfg">GeeksforGeeks (gfg)</option>
                      <option value="git">GitHub (git)</option>
                      <option value="web">Web (web)</option>
                    </select>
                  </div>
                </div>

                 {/* 4. DOCX-like Rich Text Notes Editor */}
                <div className="input-group" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: '180px' }}>
                  <label className="input-label" style={{ fontWeight: 'bold', fontSize: '12px', display: 'block', marginBottom: '4px' }}>
                    Notes
                  </label>
                  ...
                  >
                    <button 
                      ...
                      style={{ 
                        ...
                        width: '28px', 
                        height: '28px',
                        backgroundColor: editorState.bold ? '#e8f0fe' : 'transparent',
                        color: editorState.bold ? '#1a73e8' : 'var(--text)',
                        border: editorState.bold ? '1px solid #1a73e8' : 'none'
                      }}
                      title="Bold"
                      onMouseDown={(e) => {
                        e.preventDefault();
                    ...
                    <button 
                      ...
                      style={{ 
                        ...
                        width: '28px', 
                        height: '28px',
                        backgroundColor: editorState.italic ? '#e8f0fe' : 'transparent',
                        color: editorState.italic ? '#1a73e8' : 'var(--text)',
                        border: editorState.italic ? '1px solid #1a73e8' : 'none'
                      }}
                      title="Italic"
                      onMouseDown={(e) => {
                        e.preventDefault();
                    ...
                    <button 
                      ...
                      style={{ 
                        ...
                        width: '28px', 
                        height: '28px',
                        backgroundColor: editorState.underline ? '#e8f0fe' : 'transparent',
                        color: editorState.underline ? '#1a73e8' : 'var(--text)',
                        border: editorState.underline ? '1px solid #1a73e8' : 'none'
                      }}
                      title="Underline"
                      onMouseDown={(e) => {
                        e.preventDefault();
                    ...
                    {/* Single Attachment Paperclip Button (Accepts all audio/video/images, no words) */}
                    <button 
                      type="button" 
                      className="header-btn" 
                  ...
                  {/* Document Contenteditable Area with onPaste handler */}
                  <div
                    ref={editorRef}
                    contentEditable
                    onPaste={handlePaste}
                    ...
                    onClick={updateEditorActiveStates}
                    placeholder="Write notes here... type emojis, format text, or insert attachments."
                    style={{
                      flex: 1,
                      border: '1px solid var(--border)',
                    ...
                    onBlur={() => {
                      if (editorRef.current) {
                        setEditNotes(editorRef.current.innerHTML);
                      }
                    }}
                  />
                </div>
        ...
        {newTagOpen && (
          ...
          >
            <div 
              ...
              style={{ height: 'auto', maxWidth: '400px', margin: 'auto', borderTopLeftRadius: '16px', borderTopRightRadius: '16px', borderBottomLeftRadius: '16px', borderBottomRightRadius: '16px' }}
